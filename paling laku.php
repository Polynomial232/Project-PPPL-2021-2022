...
<div class="position-fixed wa">
    ...
    <div class="container-xxl px-4">
        ...
        <div id="menu" class="text-uppercase text-center">
            ...
            <div class="row">
				<?php 
					$brgs=mysqli_query($conn,"SELECT * from produk ORDER BY rate DESC");
					$no=1;
					while($p=mysqli_fetch_array($brgs)){
            $idproduk = $p['idproduk'];
            $q = mysqli_query($conn,"SELECT * from testimoni WHERE `id_produk`='$idproduk'");
            $c = mysqli_num_rows($q);
            $rate = 0;
            ...
            if($c != 0){
              $rate = $rate / $c;
              $upda = mysqli_query($conn, "UPDATE `produk` SET `rate`='$rate' WHERE `idproduk`='$idproduk'") or die($conn->error);
            }
				...
				<div class="col-sm-4">
					<a href="menu.php?id=<?= $p['idproduk'] ?>"  class="no-style">
						...
						<span class="position-absoulute d-block">
						<?php
							$star = 0;
							while($star < $p['rate']){
