<?php
	...
	$q = mysqli_query($conn, "SELECT * FROM `produk` WHERE `idproduk` ='$id'");
	$p = mysqli_fetch_array($q);
	
	$kat = $p['idkategori'];
	$q2 = mysqli_query($conn, "SELECT * FROM `kategori` WHERE `idkategori`='$kat'");
    ...
    <div class="container-xxl px-4">
        ...
        <div id="menu" class="text-uppercase">
			<section id="main" class="container">
				<div class="row">
					<div class="col-md-4">
						<img src="<?php echo $p['gambar']?>"  alt="" class="img-fluid">
					</div>
					<div class="col-md-5">
						<h2 class="fw-bold"><?= $p['namaproduk'] ?></h2>
						<h5>Kategori : <a href="produk.php?idkategori=<?= $kat ?>"  class="no-style"><?= $p2['namakategori'] ?></a></h5>
						<?php
							$star = 0;
							while($star < $p['rate']){
						...
						?>
						<p class="mt-3"><?= $p['deskripsi'] ?></p>
						<h4 class="text-capitalize">
						Harga :
							<?php
								if($p['hargaafter'] != $p['hargabefore']){
									echo 'Rp '.number_format($p['hargaafter']);
									echo '<del style="margin-left: 30px">Rp '.number_format($p['hargabefore']).'</del>';
								}else{
									echo 'Rp '.number_format($p['hargabefore']);
								}
							?></h4>
						<a href="addcart.php?id=<?= $id?>" class="btn btn-success mt-4"> Masukkan Ke Keranjang </a>
					</div>
          <div class="col-md-3 mt-4">
            <?php
              $q100 = mysqli_query($conn, "SELECT * FROM `testimoni` WHERE `id_produk`='$id'");
              while($r100 = mysqli_fetch_array($q100)){
                $userid = $r100['id_user'];
                $idproduk = $r100['id_produk'];

                $q200 = mysqli_query($conn, "SELECT * FROM `login` WHERE `userid`='$userid'");
                $r200 = mysqli_fetch_array($q200);

                $star = 0;
                while($star < $r100['rate']){
                  echo '<i class="fa-solid fa-star" style="color: #FFD700;"></i>';
                  $star++;
                }
            ?>
              <br><?= $r100['deskripsi'] ?> By: <?= $r200['namalengkap'] ?> <hr>
            <?php
              }
            ?>
          </div>
