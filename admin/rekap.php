                ...
                <div class="menu-inner">
                    <nav>
                        <ul class="metismenu" id="menu">
							...
							<li class="active"><a href="rekap.php"><span>Rekap Bulanan</span></a></li>
							<li>
                    ...
                    <div class="row">
                        <div class="col-md-7">
                            <?php
                                $bulan = date('F');
                            ?>
                            <h4 class="text-center">Pemasukan Bulan <?= $bulan ?></h4>
                            <table class="table">
                                <tr>
                                    <td>No Order</td>
                                    <td>Pemasukan</td>
                            ...
                            <?php
                                $bulan = date('Y-m');
                                $total = 0;
                                $q = mysqli_query($conn, "SELECT * FROM `konfirmasi`");
                                while($r = mysqli_fetch_assoc($q)){
                                    $order = $r['orderid'];
                                    $order_t = date("d-F-Y", strtotime($r['tglsubmit']));
                                    if(date("Y-m", strtotime($r['tglsubmit'])) == $bulan){
                                        $q1 = mysqli_query($conn, "SELECT * FROM `detailorder` WHERE `orderid` = '$order'");
                                        $r1 = mysqli_fetch_assoc($q1);
                                        $produk = $r1['idproduk'];
                                        $q2 = mysqli_query($conn, "SELECT * FROM `produk` WHERE `idproduk` = '$produk'");
                                        $r2 = mysqli_fetch_assoc($q2);
                                        
                                        $temp_total = $r2['hargaafter'] * $r1['qty'];
                                        $total += $temp_total;
                                        echo '
                                            <tr>
                                                <td>'.$order_t.'</td>
                                                <td>Rp '.number_format($temp_total).'</td>
                                            </tr>';
                                    }
                                }
                            ?>
                        ...
                        <div class="col-md-5">
                            <h4 class="text-center">Pemasukan Bulan sebelumnya</h4>
                            <table class="table">
                                <tr>
                                    <td>Bulan</td>
                                    <td>Pemasukan</td>
                            ...
                            <?php
                                $bulan = date('Y-m');
                                $total = 0;
                                $q = mysqli_query($conn, "SELECT * FROM `konfirmasi` ORDER BY `tglsubmit`");
                                $strm = "";
                                $rekap_p = array();
                                $bulan_p = array();
                                while($r = mysqli_fetch_assoc($q)){
                                    $order = $r['orderid'];
                                    $strdate = date("Y-m", strtotime($r['tglsubmit']));
                                    if($strdate < $bulan){
                                        $q1 = mysqli_query($conn, "SELECT * FROM `detailorder` WHERE `orderid` = '$order'");
                                        $r1 = mysqli_fetch_assoc($q1);
                                        $produk = $r1['idproduk'];
                                        ...
                                        $temp_total = $r2['hargaafter'] * $r1['qty'];
                                        $total += $temp_total;
                                        array_push($rekap_p, $temp_total);
                                        array_push($bulan_p, $strdate);
                                        if($strm == $strdate){
                                            array_pop($rekap_p);array_pop($rekap_p);
                                            array_pop($bulan_p);
                                            $temp_total = $r2['hargaafter'] * $r1['qty'] + $total_bulan;
                                            array_push($rekap_p, $temp_total);
                                        }
                                        $total_bulan = $temp_total;
                                        $strm = $strdate;
                                    }
                                }
                                $i = 0;
                                foreach($rekap_p as $data){
                                    $strtime = date("F-Y", strtotime($bulan_p[$i]));
                                    echo '
                                    <tr>
                                        <td>'.$strtime.'</td>
